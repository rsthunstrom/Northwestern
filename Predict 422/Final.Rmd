...
svmfit=svm(y~., data=testdat, kernel="linear", cost=.01, scale=FALSE, decision.values = T, probability = TRUE)
fitted=attributes(predict(svmfit,testdat,
                          decision.values=TRUE))$decision.values

rocplot(fitted, testdat$y, main="Training Data")
```
...
ypred
predsvm <- prediction(ypred,testdat$y)
perfsvm <- performance(predsvm,"tpr","fpr")
plot(perfsvm, main ="ROC Curve for SVM")


#predicting with sub-optimal model C = 0.01

ypred=predict(svmfit ,testdat)
table(predict=ypred, truth=testdat$y)

pred.svm=attributes(predict(svmfit,dat,
                          decision.values=TRUE))$decision.values
...
#create linearly seperated data for SVM
x[y==1,]=x[y==1,]+0.5
plot(x, col=(y+5)/2, pch=19)

#fit SVM with large cost so we get no misclassified obs
dat=data.frame(x=x,y=as.factor(y))
svmfit=svm(y~., data=dat, kernel="linear", cost=1)
summary(svmfit)
plot(svmfit, dat)

#making the margin so small most likely overfits this data
#relax cost to make a more optimal/predictive model Cost = 1
svmfit=svm(y~., data=dat, kernel="linear", cost=1)
summary(svmfit)
plot(svmfit ,dat)
ypred=predict(svmfit ,testdat)
...
#tune to find best value for cost
set.seed (1)
...
table(true=dat[-train,"y"], 
      pred=predict(svmfit,newx=dat[-train ,]))

fitted=attributes(predict(svmfit.opt,dat[train,],
                          decision.values=TRUE))$decision.values

#use fitted values to plot ROC curve
par(mfrow=c(1,2))
rocplot(fitted ,dat[train ,"y"],main="Training Data")

#jack up gamma to increase accuracy on training data
#need to be cogniziant of this impact on test data, overfit 101
