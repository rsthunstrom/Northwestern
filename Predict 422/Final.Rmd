...
In this graphic, the heavily shaded blue boxes are indicating high positive correlations and the heavily shaded red boxes are a representation of a negative correlation.  This correlogram shows us a few things we might except, for example if someone donates, their donation amount is positive, so the dataset seems to be loaded correctly.  Additionaly, the regions have a negative correlation with the other regions since each potential donor can only be located in one region.  The income features are correlated to donor and donor amount as well.  The features with the greatest correlation (positive or negative) with donor and donor amount appear to be the features indicating home ownership and the number of children in the household. 
...
Clearly, we are seeing some skew with this distribution as evidenced by the fat tails on the far right.  Typically a log transformation can take care of this type of skewness.  Let's transform each of the income features with a log to verify that we have a created a more normal distribution which will be beneficial for our modeling.
...
Let's examine the dollar donation features to see if we need to transform them like we did with the other currency related features.
...
The maximum profit for the LDA model is $11,355.50 acheived by mailing the first 1,391 potential donors of the total 2,018 in the validation dataset. The graph does a great job at visualizing the maximum profit.  The profit is steadily rising until we hit the 1,391st potential donor, then it starts to drop telling us that any mailing past that point is a waste.
...
The model appears to producing very similar results to the QDA and LDA models.  When the distribution of the predictor variables are assumed to be normal, this is common (Section 4.4, James, Witten, Hastie, Tibshirani)
...
The logistic model produces a maximum profit slightly higher than the other models at $11,390.50 and a circulation of 1446.  This is telling us that this model does a better job at recalling the positive responses, which is the objective of the model and the way in which it produces higher profit since the benefit outweighs the cost (14.50 > 2.00)
...
As we suspected, the profit is only $10,501.50 from a circulation quantity of 1,818.  
...
Even though the SVM with RBF kernel has good performance, it appears that it's not as strong as the random forest model.  Generating the maximum profit we find that in fact, it's not quite as good as the random forest.  The maximum profit for this model is only $10,981.00

```{r include=FALSE}
# calculate ordered profit function using average donation = $14.50 and mailing cost = $2
profit.svm.rbf <- cumsum(14.5*c.valid[order(pred.svm.rbf, decreasing=T)]-2)
...
Logsitic Regression, L2 Regularization | $11,393.00
Logistic Regression | $11,390.50
Logsitic Regression, L1 Regularization | $11,379.50
Linear Discriminant Analysis | $11,355.50
Quadractic Discriminant Analysis | $11,205.00
k-NN Classification | $11,154.00
SVM, Radial Basis Function | $10,981.00
SVM, Linear | $10,501.50

Here we can see that the random forest model produces the maximum profit. 
